  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onDiscard()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Vista previa de la acción de IA</DialogTitle>
          <DialogDescription>Revisa el resultado antes de aplicarlo al artículo.</DialogDescription>
        </DialogHeader>

        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Texto original</p>
            <div className="max-h-64 overflow-y-auto rounded-md border bg-muted/30 p-3 text-sm">{originalText}</div>
          </div>
          <div>
            <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Texto propuesto</p>
