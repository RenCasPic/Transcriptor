...
1. **Proveedores** (`src/lib/ai/provider.ts` + `src/lib/ai/providers/*`): implementan `ContentGenerationProvider` y `TranscriptionProvider`. Hoy existen `MockContentGenerationProvider` (determinista, sin costo) y `GenericContentGenerationProvider` (Anthropic/OpenAI vía `fetch`, sin SDKs adicionales). El proveedor activo se resuelve por variable de entorno (`AI_PROVIDER`) en `src/lib/ai/providers/index.ts`.
...
- El proveedor mock y las llamadas a Anthropic/OpenAI para un artículo típico responden en segundos, no minutos — aceptable dentro del timeout de una función serverless.
