...
- Si el video no tiene subtítulos (ni manuales ni generados), se extrae su audio con **yt-dlp** (`transcribeYoutubeAudioAction`) y se transcribe con el proveedor configurado. Requiere `TRANSCRIPTION_API_KEY` / `GROQ_API_KEY`; sin clave, falla con un mensaje explícito.
...
- El fallback de audio topa en `MAX_MEDIA_BYTES` (25 MB) porque `transcribeYoutubeAudioAction` no pasa por el troceado con ffmpeg (eso es solo para archivos subidos). Un audio m4a de ~25 min llena ese tope. Videos más largos sin subtítulos siguen necesitando "Subir archivo".

**Estado del fallback de audio (2026)**: las librerías JS puras (`@distube/ytdl-core` —archivada el 2025-08-16— y `youtubei.js`) **ya no sirven para descargar**: YouTube pasó la entrega a **SABR** (streaming ABR del lado servidor sobre UMP) y no expone URLs de formato; un PO Token no lo arregla (verificado). El extractor por defecto es **`YtDlpAudioExtractor`** (`src/lib/integrations/audio-extractor/ytdlp-extractor.ts`): invoca el binario `yt-dlp` (lo descarga `scripts/setup-ytdlp.mjs` a `bin/` en postinstall/prebuild; viaja en el bundle de Vercel vía `outputFileTracingIncludes`). yt-dlp resuelve SABR + varios clientes de fallback y se parchea cada 1-2 semanas — si YouTube rompe la extracción, sube `YTDLP_VERSION`. `AUDIO_EXTRACTOR=ytdl-core` fuerza el extractor JS anterior (archivado, casi siempre falla). El binario nunca escribe a disco: `yt-dlp -o -` → stdout → `readStreamWithLimit` → transcripción. Errores de yt-dlp mapeados a los mismos códigos (`YOUTUBE_EXTRACTOR_BLOCKED` para 403/bot-check, `YOUTUBE_PRIVATE_VIDEO`, `YOUTUBE_MEMBERS_ONLY`, etc.) + logging estructurado (`extraction-logger.ts`, sin cookies/tokens/URLs firmadas).

Siguiente nivel de robustez (si YouTube empieza a bloquear la IP de Vercel): mover `YtDlpAudioExtractor` a un worker aparte (Fly/Railway) con yt-dlp autoactualizado + un proveedor de PO Token (`bgutil`) + proxy residencial. La interfaz `AudioExtractor` ya aísla ese cambio.
